    ...
    <div class="page">
      <div class="page__head">
        <div class="container page__container">
          ...
          <div class="page__breadcrumbs"><a class="page__link" href="/expense"><i class="fa fa-arrow-left" aria-hidden="true" style="font-size: 17px;"></i></a>
            <svg class="icon icon-arrow-next">
              <use xlink:href="#icon-arrow-next"></use>
            </svg>
            <div class="page__text">Add Expense</div>
          </div>
          <div class="page__line">
            ...
            <div class="page__nav"><a class="page__link active" href="{{ route('expense.index') }}">Expense</a><a class="page__link" href="{{ route('budget.index') }}">Budget</a></div>
            ...
            <form method="post" action="{{ route('expense.store') }}" enctype="multipart/form-data">
                ...
                <div class="col-md-4 form-group">
                    <h5> <label for="date" class="fw-bold">Date</label></h5>
                     <input type="date" class="form-control @error('end_date') is-invalid @enderror" name="date" id="date" >
                     @error('date')
                   ...
                   <div class="col-md-4 form-group">
                    ...
                    <select name="acc_id" id="acc_id" class="form-control @error('acc_id') is-invalid @enderror">
                         @foreach ($accounts->accounts as $account)
                              <option value="{{ $account->id }}">{{ $account->name }}  ({{ getTypeName($account->type) }})</option>
                         @endforeach
                    </select>
                    @error('acc_id')
                    <span style="color: red">{{ $message }}</span>
                ...
                <div class="col-md-4 form-group">
                    ...
                    <textarea  rows="2"  class="form-control  @error('date') is-invalid @enderror"  name="description"></textarea>
                    @error('description')
                    <span style="color: red">{{ $message }}</span>
                    @enderror
                   </div>
                   ...
                   <div class="col-md-4 form-group">
                    <h5> <label for="receipt" class="fw-bold">Receipt</label></h5>
                    <input type="file" class="form-control" @error('date') is-invalid @enderror  name="receipt" id="receipt" >
                    @error('receipt')
                    <span style="color: red">{{ $message }}</span>
                    @enderror
                   </div>
                   ...
                   <div class="col-md-4">
                <button type="submit" class="mt-4 btn btn-danger">Add Expense</button>
            </div>
              </form>
            </div>
