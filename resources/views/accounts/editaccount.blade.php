    ...
    <div class="page">
      <div class="page__head">
        <div class="container page__container">
          ...
          <div class="page__breadcrumbs"><a class="page__link" href="/accounts">
            <i class="fa fa-arrow-left" aria-hidden="true" style="font-size: 17px;"></i></a>
            <svg class="icon icon-arrow-next">
              <use xlink:href="#icon-arrow-next"></use>
            </svg>
            <div class="page__text">Edit Account</div>
          </div>
      ...
      <div class="page__body">
        <div class="container page__container">
          ...
          <div class="transactions">
            <form method="post" action="{{ route('accounts.update',$accounts->id) }}">
                @csrf
                @method('PUT')
                <div class="mt-4 row">
                <div class="form-group col-md-4">
                 <h5> <label for="name" class="fw-bold">Account Name</label></h5>
                  <input type="text" class="form-control  @error('name') is-invalid @enderror" name="name" id="accname" placeholder="Enter Name" value="{{ $accounts->name }}">
                  <div>@error('name')
                ...
                @enderror
                 ...
                 <select name="type" id="type" class="form-control">
                    <option value="0"  {{ $accounts->type == 0 ? 'selected' : '' }} class="form-control">Asset</option>
                    <option value="1" {{ $accounts->type == 1 ? 'selected' : '' }} class="form-control">Liability</option>
                    <option value="2" {{ $accounts->type == 2 ? 'selected' : '' }} class="form-control">Income</option>
                    <option value="3"  {{ $accounts->type == 3 ? 'selected' : '' }}class="form-control">Expense</option>
                 </select>
                   </div>
                    <div class="col-md-4">
                <button type="submit" class="mt-4 btn btn-success">Update Account</button>
            </div>
              </form>
            </div>
