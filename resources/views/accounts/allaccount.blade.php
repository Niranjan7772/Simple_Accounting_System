      ...
      <div class="page__head">
        <div class="container page__container">
          ...
          <div class="page__breadcrumbs"><a class="page__link" href="/d2">Dashboard</a>
            ...
            <div class="page__text">Account</div>
          </div>
          <div class="page__line">
            <div class="page__nav">
                <div class="page__nav"><a class="page__link active" href="{{ route('accounts.index') }}">Account</a><a class="page__link " href="{{ route('transfer.index') }}">Transfer</a><a class="page__link " href="{{ route('transaction.index') }}">Transaction</a></div>
            </div>
                ...
                <div class="row">
                    @if (Auth::user()->hasPermission('add_account'))
                      <div class="mb-4 col-md-1">
                          <a class="page__link"  href="{{ route('accounts.create') }}"> <button class="btn btn-dark">Add Account</button></a>
                      </div>
                      @endif
               </div>
               <div id="accounts-container">
                  <table class="table mt-3 table-stripped" id="MyTable">
                             ...
                             @if ($account->type==0)
                                   ...
                                   @elseif ($account->type==2)
                                    ...
                                    @else
                                        <td>Expense</td>
                             @endif
                            <td>  @if (Auth::user()->hasPermission('edit_account'))
                                <a href="{{ route('accounts.edit',$account->id) }}" type="button">
                                    <i class="fa fa-pencil-square-o" style="font-size:30px;color:green" ></i>
                                  </a>
                                    @endif
                                    @if (Auth::user()->hasPermission('delete_account'))
                                <a type="button"  data-bs-toggle="modal" data-bs-target="#exampleModal{{ $account->id }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#pagination-links a').on('click', function (e) {
                ...
                // Fetch and update the content using AJAX
                $.ajax({
                    url: url,
                    type: 'GET',
                    success: function (data) {
                        $('#accounts-container').html($(data).find('#accounts-container').html());
