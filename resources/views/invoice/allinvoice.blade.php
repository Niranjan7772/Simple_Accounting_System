...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet"  media="print"  href="{{ asset('css/print-styles.css') }}">
      ...
      <div class="page__head">
        <div class="container page__container">
          ...
          <div class="page__breadcrumbs"><a class="page__link" href="/d2">Dashboard</a>
            ...
            <div class="page__text">Invoice</div>
          </div>
          <div class="page__line">
            <div class="page__nav">
                <div class="page__nav"><a class="page__link active" href="{{ route('invoice.index') }}">Invoice</a><a class="page__link " href="{{ route('customer.index') }}">Customer</a></div>
            </div>
         ...
         <div class="alert alert-danger">
            ...
            <div class="row">
                  ...
                  <div class="mb-3 col-md-1">
                      <a class="page__link"  href="{{ route('invoice.create') }}"> <button class="btn btn-danger">Add Invoice</button></a>
                  </div>
           </div>
              <table class="table mt-3 table-striped" id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                         ...
                         <td>{{ $invoice->status == 1 ? 'Outstanding' : 'Paid' }}</td>
                         <td>
                           <!-- Button trigger modal -->
                           @if($invoice->status == 1)
                           {{-- <a href="{{ route('invoice.edit',$invoice->id) }}"><button class="btn btn-primary">Edit</button></a> --}}

<button  data-toggle="modal" class="btn btn-success"    data-target="#exampleModal{{ $invoice->id }}" {{ $invoice->status==0 ?'disabled':'' }} >
   Pay
  </button>
  @endif
  <button class="Button Button--outline" onclick="fetchAndPrint({{ $invoice->id }})"><i class="fa fa-print" style="font-size:30px;color:rgb(255, 174, 0)"></i></button>
  ...
  <div class="modal fade" id="exampleModal{{ $invoice->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
           <h5> <label class="form-label">Select Account</label></h5><br>
           <input type="hidden" name="amount" value="{{ $invoice->amount }}">
        <select class="form-control" name="acc_id" style="width: 450px">
              ...
              <div id="printableTable" style="display: none;">
                <table  class="order-details" style="width: 100%;">
                    ...
                    font-family: sans-serif;">
                        ...
                        <tr class="bg-blue"   style=" background-color: #414ab1;
                        color: #fff;">
                ...
                <table style="width: 100%;"  >
                    <thead >
                        <tr>
                            <th class="no-border text-start heading" colspan="5" style=" font-size: 24px;
                            margin-top: 12px;
                            margin-bottom: 12px;
                    ...
                    <tbody  id="orderItemsContainer" style="text-align: center;">
                    </tbody>
                    <tfoot style="">
                        <td colspan="4" class="total-heading" style=" font-size: 18px;
                        font-weight: 700;
                        font-family: sans-serif;">Total Amount  :</td>

                        <td colspan="1" class="total-heading" id="totalAmount" style= "font-size: 18px;
                        font-weight: 700;
                        text-align: center;
                        paddind:10px;
                        font-family: sans-serif;">{{ $invoice->amount }}</td>
                    </tfoot>
                </table>
    ...
    function fetchAndPrint(invoiceId) {
        fetch(`/invoice/print/${invoiceId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`Error fetching invoice details: ${response.status} ${response.statusText}`);
            }
            return response.json();
        })
        .then(data => {
            updatePrintableTable(data);
            printDiv();
...
function updatePrintableTable(data) {


    updateElement('invoiceIdPrint', data.invoiceId);
    updateElement('fullName', data.fullName);
    updateElement('issueDate', data.issueDate);
    updateElement('emailId', data.emailId);
    updateElement('dueDate', data.dueDate);
    updateElement('phone', data.phone);
    updateElement('status', data.status === 1 ? 'Outstanding' : 'Paid');
    ...
    updateElement('totalAmount', data.totalAmount);
}
